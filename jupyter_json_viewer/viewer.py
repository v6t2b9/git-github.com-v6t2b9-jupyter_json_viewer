...
def display_json(
    ...
    def format_value(v: Any, depth: int = 0, path: str = '') -> str:
        ...
        if isinstance(v, dict):
            return format_dict(v, depth, path)
        elif isinstance(v, list):
            return format_list(v, depth, path)
    ...
    def format_dict(d: dict, depth: int, path: str) -> str:
        ...
        for i, (k, v) in enumerate(d.items()):
            is_first = i == 0
            is_last = i == len(d) - 1
            marker = "┌" if is_first else "└" if is_last else "├"
            ...
            result.append(
                ...
                f'{format_value(v, depth + 1, current_path)}'
                f'</div>'
            )
    ...
    def format_list(lst: list, depth: int, path: str) -> str:
